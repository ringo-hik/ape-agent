...
import json
sys.path.append('.')
from config.models_config import get_models, get_default_model, get_default_backup_model

# 모델 정보 가져오기
models = get_models()
default_model = get_default_model()
default_backup = get_default_backup_model()

# 결과 출력
print(f'\n모델 정보 테스트:')
print(f'- 사용 가능한 모델 수: {len(models)}')
print(f'- 기본 모델: {default_model}')
print(f'- 백업 모델: {default_backup}')
print('\n모델 목록:')
